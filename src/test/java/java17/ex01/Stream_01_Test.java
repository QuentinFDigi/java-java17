...
public class Stream_01_Test {
	...
	public void test_stream_findFirst() throws Exception {
		...
		Optional<Order> result = orders.stream().filter(order -> Objects.equals(order.getCustomer().getFirstname(), "Sophie")).findFirst();
	...
	public void test_stream_filter_adv() throws Exception {
		...
		// pizza Pépéroni
		List<Order> result = orders.stream().filter(order -> order.getPizzas().stream().allMatch(pizza -> Objects.equals(pizza.getName(), "Pépéroni"))).toList();
